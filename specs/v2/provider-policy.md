...
Both `action` and `resource` use zaovra's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.zaovra` files is intentionally deferred until `.zaovra` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether zaovra permits an operation through its managed execution paths.
