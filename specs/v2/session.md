...
| Per-turn request assembly  | Selected agent, agent prompt, and effective permissions                  | complete | Selected-agent instructions, effective permissions, model defaults and request settings are applied. See preflight-completion-review.md.                      |
| Per-turn request assembly  | Provider/model-specific base instructions                                | complete | A provider-neutral baseline and per-request provider/model identity are supplied; model-family specialization is intentionally excluded for BYOK.             |
...
| Per-turn request assembly  | Model variants and request settings                                      | partial  | Effective agent options and selected variants are applied. Plugin-mutated settings remain a separate API design.                                              |
...
| Prompt/reference expansion | File, directory, media, and MCP-resource materialization                 | partial  | Local files/directories/media use permissions and replay snapshots; unsupported remote/MCP URLs report explicit failures. See preflight-completion-review.md. |
