# Remove `packages/zaovra/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/zaovra/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the zaovra legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/zaovra/src/storage/db.ts` are concentrated in 22 source files:

- `packages/zaovra/src/account/repo.ts`
- `packages/zaovra/src/cli/cmd/db.ts`
- `packages/zaovra/src/cli/cmd/import.ts`
- `packages/zaovra/src/cli/cmd/stats.ts`
- `packages/zaovra/src/control-plane/workspace.ts`
- `packages/zaovra/src/index.ts`
- `packages/zaovra/src/node.ts`
- `packages/zaovra/src/permission/index.ts`
- `packages/zaovra/src/project/project.ts`
- `packages/zaovra/src/server/projectors.ts`
- `packages/zaovra/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/zaovra/src/server/shared/fence.ts`
- `packages/zaovra/src/session/message-v2.ts`
- `packages/zaovra/src/session/projectors.ts`
- `packages/zaovra/src/session/prompt.ts`
- `packages/zaovra/src/session/session.ts`
- `packages/zaovra/src/session/todo.ts`
- `packages/zaovra/src/share/share-next.ts`
- `packages/zaovra/src/storage/db.ts`
- `packages/zaovra/src/sync/index.ts`
- `packages/zaovra/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy zaovra database wrapper; `packages/zaovra/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the zaovra wrapper.
...
Status: Completed. `SyncEvent` and the zaovra projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy zaovra database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy zaovra database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy zaovra database wrapper.
...
All migration groups are complete or superseded. `packages/zaovra/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No zaovra data-migration group remains.

The previous zaovra `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate zaovra data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/zaovra`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/zaovra/src`
- `bun typecheck` from `packages/zaovra`
- Relevant package tests from `packages/zaovra`, not the repo root
