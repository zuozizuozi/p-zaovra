...
Move the canonical Zaovra terminal application from
`packages/zaovra/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/zaovra`,
`packages/cli`, or `@zaovra-ai/core`.

The SDK is the TUI's Zaovra boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/zaovra`.
- Do not replace private `packages/zaovra` imports with `@zaovra-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@zaovra-ai/tui`.
...
- Zaovra domain data displayed by the TUI
...
```text
packages/zaovra/src/cli/cmd/tui
```

Its private dependency on `packages/zaovra` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/zaovra/src/*`.
...
Status: Completed. The private `@zaovra-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@zaovra-ai/tui`.
...
- The package has no dependency on `zaovra`, `@zaovra-ai/cli`, or
  `@zaovra-ai/core`.
...
primitives now live in `@zaovra-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@zaovra-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Zaovra globals or process state;
...
Replace process-global Zaovra state with resolved TUI inputs.
...
`@zaovra-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/zaovra` initially.
...
canonical in `@zaovra-ai/tui`. Configured references resolve through the new
...
adapters rather than Zaovra domain implementations.

Make the SDK the only Zaovra domain boundary used by the TUI.
...
- Domain-facing TUI code imports Zaovra data and operations only from
  `@zaovra-ai/sdk`.
...
`@zaovra-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/zaovra`.
...
- The package has no imports from `packages/zaovra`, `packages/cli`, or
  `@zaovra-ai/core`.
...
public `@zaovra-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/zaovra`.
- Keep the legacy embedded worker and server startup in `packages/zaovra`.
- Change those adapters to load config, create transport inputs, and call the
  public `@zaovra-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/zaovra`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@zaovra-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@zaovra-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Zaovra domain state.
...
cd packages/zaovra && bun typecheck
...
rg '@zaovra-ai/core|packages/zaovra|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/zaovra/src packages/zaovra/test
```
...
- Build the current-platform `packages/zaovra` binary.
