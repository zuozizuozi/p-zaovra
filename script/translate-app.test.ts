...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".zaovra/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".zaovra/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".zaovra/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "zaovra/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "zaovra/gpt-5.5")).toEqual({
