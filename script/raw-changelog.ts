...
const repo = process.env.GH_REPO ?? "zuozizuozi/p-zaovra"
const bot = ["actions-user", "github-actions[bot]", "zaovra", "zaovra-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/zaovra packages/sdk packages/plugin packages/desktop-app packages/app-ui packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/zaovra/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/zaovra/")) areas.add("core")
      else if (file.startsWith("packages/desktop-app/src-tauri/") || file.startsWith("packages/desktop/src-tauri/")) areas.add("tauri")
      else if (["packages/desktop-app/", "packages/app-ui/", "packages/desktop/", "packages/app/"].some((prefix) => file.startsWith(prefix))) areas.add("app")
