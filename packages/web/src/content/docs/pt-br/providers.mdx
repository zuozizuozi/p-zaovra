---
title: Provedores
description: Usando qualquer provedor LLM no zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

zaovra usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do zaovra.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/zaovra/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do zaovra.
...
Zaovra Zen é uma lista de modelos fornecidos pela equipe do zaovra que foram testados e verificados para funcionar bem com o zaovra. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Zaovra Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Zaovra Zen` e acesse [zaovra.com/zen](https://zaovra.com/zen).
...
Funciona como qualquer outro provedor no zaovra e é completamente opcional.

---

## Zaovra Go

Zaovra Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do zaovra que foram
testados e verificados para funcionar bem com o zaovra.

1. Execute o comando `/connect` no TUI, selecione `Zaovra Go` e acesse [zaovra.com/zen](https://zaovra.com/zen).
...
Para usar o Amazon Bedrock com o zaovra:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o zaovra:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `zaovra.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no zaovra não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o zaovra para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o zaovra funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do zaovra.
...
armazenar o token no armazenamento de autenticação do zaovra.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
zaovra usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o zaovra
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `zaovra.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth).

##### Configuração

Personalize através do `zaovra.json`:
...
Para usar sua assinatura do GitHub Copilot com o zaovra:
...
Para usar o Google Vertex AI com o zaovra:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o zaovra.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do zaovra, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session) para registrar automaticamente cada conversa do zaovra como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do zaovra listada como uma sessão separada.
...
Você pode configurar o zaovra para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o zaovra para usar modelos locais através do LM Studio.
...
Você pode configurar o zaovra para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o zaovra. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/zaovra) para detalhes.
:::
...
Para usar o Ollama Cloud com o zaovra:
...
3. Copie a chave da API para uso no zaovra.
...
6. **Importante**: Antes de usar modelos em nuvem no zaovra, você deve puxar as informações do modelo localmente:
...
Zaovra Zen é uma lista de modelos testados e verificados fornecidos pela equipe do zaovra. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Zaovra Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Zaovra Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do zaovra.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do zaovra.
...
5. Você também pode personalizá-los através da sua configuração do zaovra. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o zaovra:
...
Você também pode personalizar modelos através da sua configuração do zaovra. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o zaovra. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no zaovra.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `zaovra.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o zaovra entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `zaovra auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do zaovra e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do zaovra.
