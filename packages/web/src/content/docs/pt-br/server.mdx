---
title: Servidor
description: Interaja com o servidor zaovra via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `zaovra serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente zaovra pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `zaovra.local` |
...
Defina `ZAOVRA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `zaovra`, ou defina `ZAOVRA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `zaovra serve` quanto ao `zaovra web`.
...
Quando você executa `zaovra`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor zaovra para interagir com o zaovra programaticamente.
:::

Essa arquitetura permite que o zaovra suporte múltiplos clientes e permite que você interaja com o zaovra programaticamente.

Você pode executar `zaovra serve` para iniciar um servidor autônomo. Se você tiver o TUI do zaovra em execução, `zaovra serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do zaovra [IDE](/docs/ide).
...
O servidor zaovra expõe as seguintes APIs.
