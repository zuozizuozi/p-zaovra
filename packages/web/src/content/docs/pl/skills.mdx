...
Umiejętności agenta pozwalają zaovra odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
zaovra przeszukuje te lokalizacje:

- Project config: `.zaovra/skills/<name>/SKILL.md`
- Global config: `~/.config/zaovra/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu zaovra przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.zaovra/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.zaovra/skills/git-release/SKILL.md` w ten sposób:
...
zaovra wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `zaovra.json`:
...
**Dla agentów wbudowanych** (w `zaovra.json`):
