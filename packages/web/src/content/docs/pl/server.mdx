---
title: Serwer
description: Interakcja z serwerem zaovra za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Komenda `zaovra serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient zaovra.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `zaovra.local` |
...
Ustaw `ZAOVRA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `zaovra` lub ustaw `ZAOVRA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `zaovra serve`, jak i `zaovra web`.
...
Kiedy uruchomisz `zaovra`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera zaovra do programowej interakcji z zaovra.
:::

Ta architektura umożliwia obsługę wielu klientów przez zaovra i programową interakcję z kodem otwartym.

Możesz uruchomić `zaovra serve`, aby uruchomić samodzielny serwer. Jeśli masz
zaovra TUI działa, `zaovra serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki zaovra [IDE](/docs/ide).
...
Serwer zaovra udostępnia następujące interfejsy API.
