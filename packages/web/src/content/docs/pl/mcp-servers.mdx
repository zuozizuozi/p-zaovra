...
Możesz dodać dodatkowe informacje do zaovra za pomocą _Model Context Protocol_, w skrócie MCP. zaovra obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [zaovra Config](https://zaovra.com/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/zaovra`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
zaovra automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, zaovra:
...
Jeśli serwer wymaga uwierzytelnienia, zaovra poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `zaovra mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji zaovra bezpieczne przechowa tokeny w `~/.local/share/zaovra/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w zaovra, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie zaovra, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć zaovra z kontem Sentry.
