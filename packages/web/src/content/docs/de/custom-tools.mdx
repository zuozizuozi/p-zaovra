...
description: Erstellen Sie Tools, die der LLM in Zaovra aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Zaovra wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.zaovra/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/zaovra/tools/` platzieren.
