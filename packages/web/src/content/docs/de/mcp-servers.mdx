...
Sie können externe Tools zu Zaovra hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Zaovra unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Zaovra Config](https://zaovra.com/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/zaovra`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Zaovra übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Zaovra Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Zaovra beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `zaovra mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Zaovra die Token sicher in `~/.local/share/zaovra/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Zaovra verfügbar. Sie können sie also wie jedes andere Tool über die Zaovra-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Zaovra mit Ihrem Sentry-Konto zu verbinden.
