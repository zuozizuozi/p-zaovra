---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Zaovra.
---
...
2. Konfigurieren Sie den Anbieter in Ihrer Zaovra-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Zaovra anpassen
config.
...
Zaovra Zen ist eine Liste von Modellen, die vom Zaovra-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Zaovra funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Zaovra Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in Zaovra und ist völlig optional.

---

## Zaovra Go

Zaovra Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Zaovra-Team getestet und verifiziert wurden, dass sie gut mit Zaovra funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Zaovra Go` und gehen Sie zu [zaovra.com/auth](https://zaovra.com/zen).
...
So verwenden Sie Amazon Bedrock mit Zaovra:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Zaovra ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `zaovra.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Zaovra wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können zaovra so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Zaovra ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Zaovra-Konfiguration hinzufügen.
...
um das Token im Zaovra-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Zaovra verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Zaovra
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`zaovra.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `zaovra.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Zaovra:
...
So verwenden Sie Google Vertex AI mit Zaovra:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Zaovra ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Zaovra konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session)-Plugin, um jede Zaovra-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Zaovra-Konversation als separate Sitzung aufgeführt.
...
Sie können Zaovra für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Zaovra über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Zaovra über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Zaovra konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/zaovra).
:::
...
So verwenden Sie Ollama Cloud mit Zaovra:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Zaovra.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Zaovra verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Zaovra Zen ist eine Liste getesteter und verifizierter Modelle, die vom Zaovra-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Zaovra Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Zaovra Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Zaovra API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Zaovra-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Zaovra-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Zaovra:
...
Sie können Modelle auch über Ihre Zaovra-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Zaovra verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `zaovra.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Zaovra erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `zaovra auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Zaovra-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Zaovra-Konfiguration übereinstimmt.
