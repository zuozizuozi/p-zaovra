...
Agent Skills erlauben Zaovra, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Zaovra sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Zaovra vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.zaovra/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.zaovra/skills/git-release/SKILL.md` zum Beispiel so:
...
Zaovra listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `zaovra.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `zaovra.json`):
