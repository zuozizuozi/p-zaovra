---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Zaovra zu erweitern.
---

Mit Plugins können Sie Zaovra erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Zaovra zu ändern.
...
- `.zaovra/plugins/` – Plugins auf Projektebene
- `~/.config/zaovra/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/zaovra/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/zaovra/zaovra.json`)
2. Projektkonfiguration (`zaovra.json`)
3. Globales Plugin-Verzeichnis (`~/.config/zaovra/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.zaovra/plugins/`)
...
Zaovra führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Zaovra-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Zaovra erweitern können.
...
:::note
Wenn Sie die Zaovra-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Zaovra `.env`-Dateien liest:
...
Plugins können Zaovra auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Zaovra aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Zaovra zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://zaovra.com/docs/sdk).
