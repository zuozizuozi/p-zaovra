...
Avec Zaovra, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Zaovra utilise notre propre thème `zaovra`.
...
Zaovra est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Zaovra corresponde à l'apparence de leur terminal
...
Zaovra prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/zaovra/themes/*.json` ou `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.zaovra/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.zaovra/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/zaovra/themes
vim ~/.config/zaovra/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .zaovra/themes
vim .zaovra/themes/my-theme.json
```
...
{
  "$schema": "https://zaovra.com/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
