---
title: SDK
description: Client JS de type sécurisé pour le serveur zaovra.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK zaovra JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler zaovra par programme.
...
Créez une instance de zaovra :

```javascript
import { createZaovra } from "@zaovra-ai/sdk"

const { client } = await createZaovra()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `zaovra.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${zaovra.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de zaovra, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@zaovra-ai/sdk"
```
