...
description: Zaovra, LSP sunucularınızla bütünleşir.
---

Zaovra, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Zaovra, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`ZAOVRA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve zaovra bir dosyayı açtığında:
...
LSP sunucularını, zaovra yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
