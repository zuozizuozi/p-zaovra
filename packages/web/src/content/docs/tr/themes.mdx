...
zaovra ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak zaovra kendi `zaovra` temasını kullanır.
...
zaovra birden fazla yerleşik temayla gelir.
...
- zaovra'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
zaovra, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/zaovra/themes/*.json` veya `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.zaovra/themes/*.json`
4. **Geçerli çalışma dizini** - `./.zaovra/themes/*.json`
