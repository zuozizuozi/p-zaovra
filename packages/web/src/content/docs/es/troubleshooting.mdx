...
Para depurar problemas con Zaovra, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\zaovra\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `zaovra --log-level DEBUG`.

---

## Almacenamiento

zaovra almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/zaovra/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\zaovra`
...
Zaovra Desktop ejecuta un servidor Zaovra local (el sidecar `zaovra-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Zaovra` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/zaovra/zaovra.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\zaovra\zaovra.jsonc`
...
Zaovra también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/zaovra/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\zaovra\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Zaovra pueda reconstruirlo.

1. Salga de Zaovra Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/zaovra`
- **Linux**: eliminar `~/.cache/zaovra` (o ejecutar `rm -rf ~/.cache/zaovra`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\zaovra`

3. Reinicie el escritorio Zaovra.

---

### Solucionar problemas de conexión del servidor

Zaovra Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `zaovra.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `ZAOVRA_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `ZAOVRA_PORT` (o elija un puerto libre) y reinicie.
...
- Si estás en Wayland y la aplicación está en blanco o falla, intenta iniciarla con `ZAOVRA_ALLOW_WAYLAND=1`.
...
En Windows, el escritorio Zaovra requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Zaovra.

---

### Notificaciones que no se muestran

Zaovra Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Zaovra en la configuración de su sistema operativo, y
...
1. Salga del escritorio Zaovra.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Zaovra):

- `zaovra.settings.dat` (URL del servidor predeterminado de escritorio)
- `zaovra.global.dat` y `zaovra.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `zaovra upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `zaovra models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\zaovra`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. zaovra instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\zaovra`

2. Reinicie zaovra para reinstalar los paquetes del proveedor más recientes.

Esto obligará a zaovra a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
zaovra detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
