...
Las herramientas permiten que LLM realice acciones en su código base. Zaovra viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Zaovra.
...
:::note
Esta herramienta solo está disponible cuando `ZAOVRA_EXPERIMENTAL_LSP_TOOL=true` (o `ZAOVRA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Zaovra o cuando la variable de entorno `ZAOVRA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Zaovra:

```bash
ZAOVRA_ENABLE_EXA=1 zaovra
```
