...
Con Zaovra puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Zaovra usa nuestro propio tema `zaovra`.
...
Zaovra viene con varios temas integrados.
...
- Quiere que Zaovra coincida con la apariencia de su terminal
...
Zaovra admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/zaovra/themes/*.json` o `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.zaovra/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.zaovra/themes/*.json`
