---
title: Servidor
description: Interactuar con el servidor zaovra a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `zaovra serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente zaovra puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `zaovra.local` |
...
Configure `ZAOVRA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `zaovra`, o configure `ZAOVRA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `zaovra serve` como a `zaovra web`.
...
Cuando ejecuta `zaovra`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor zaovra para interactuar con zaovra mediante programación.
:::

Esta arquitectura permite que zaovra admita múltiples clientes y le permite interactuar con zaovra mediante programación.

Puede ejecutar `zaovra serve` para iniciar un servidor independiente. Si tienes el
zaovra TUI ejecutándose, `zaovra serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Zaovra [IDE](/docs/ide).
...
El servidor zaovra expone las siguientes API.
