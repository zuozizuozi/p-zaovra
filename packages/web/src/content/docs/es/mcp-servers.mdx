...
Puede agregar herramientas externas a Zaovra utilizando el _Model Context Protocol_ o MCP. Zaovra admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Zaovra Config](https://zaovra.com/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/zaovra`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Zaovra maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Zaovra:
...
Si el servidor requiere autenticación, Zaovra le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `zaovra mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Zaovra almacenará los tokens de forma segura en `~/.local/share/zaovra/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Zaovra, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Zaovra como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Zaovra a su cuenta Sentry.
