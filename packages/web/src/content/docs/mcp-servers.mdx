...
You can add external tools to Zaovra using the _Model Context Protocol_, or MCP. Zaovra supports both local and remote servers.
...
You can define MCP servers in your [Zaovra Config](https://zaovra.com/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/zaovra` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Zaovra automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Zaovra will:
...
If the server requires authentication, Zaovra will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `zaovra mcp auth <server-name>`.
...
The `mcp auth` command opens your browser for authorization. After approval, Zaovra stores the OAuth credential in the same encrypted V2 Credential Store used by desktop and TUI integrations; no separate `mcp-auth.json` file is created.
...
Your MCPs are available as tools in Zaovra, alongside built-in tools. So you can manage them through the Zaovra config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Zaovra to your Sentry account.
