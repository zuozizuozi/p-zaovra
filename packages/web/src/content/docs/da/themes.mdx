...
Med Zaovra kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Zaovra vårt eget `zaovra`-tema.
...
Zaovra støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/zaovra/themes/*.json` eller `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.zaovra/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.zaovra/themes/*.json`
