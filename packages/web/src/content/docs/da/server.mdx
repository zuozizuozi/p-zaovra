---
title: Server
description: Interager med zaovra-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Kommandoen `zaovra serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en zaovra-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `zaovra.local` |
...
Indstil `ZAOVRA_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `zaovra`, eller indstil `ZAOVRA_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `zaovra serve` og `zaovra web`.
...
Når du kører `zaovra` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug zaovra-serveren til at interagere med zaovra programmatisk.
:::

Denne arkitektur lader zaovra understøtte flere klienter og lader dig interagere med zaovra programmatisk.

Du kan køre `zaovra serve` for at starte en selvstændig server. Hvis du har
zaovra TUI kørende, vil `zaovra serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Zaovra [IDE](/docs/ide) plugins.
...
Zaovra-serveren viser følgende API'er.
