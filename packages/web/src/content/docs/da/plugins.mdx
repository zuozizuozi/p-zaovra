---
title: Plugins
description: Skriv dine egne plugins for at udvide Zaovra.
---

Plugins giver dig mulighed for at udvide Zaovra ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Zaovras standardadfærd.
...
- `.zaovra/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/zaovra/node_modules/`.
...
1. Global konfiguration (`~/.config/zaovra/zaovra.json`)
2. Projektkonfiguration (`zaovra.json`)
3. Global plugin-mappe (`~/.config/zaovra/plugins/`)
4. Projekt plugin bibliotek (`.zaovra/plugins/`)
...
Zaovra kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En zaovra SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide zaovra.
...
:::note
Hvis du bruger Zaovra desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå zaovra i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til zaovra:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som zaovra kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for zaovra sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://zaovra.com/docs/sdk) for detaljer.
