...
Det er to typer agenter i Zaovra; primære agenter og underagenter.
...
Zaovra kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Zaovra kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Zaovra kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Zaovra sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `zaovra.json`:
...
- Globalt: `~/.config/zaovra/agents/`
- Per prosjekt: `.zaovra/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Zaovra modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Zaovra-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Zaovra-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Zaovra Zen](/docs/zen), vil du bruke `zaovra/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `zaovra models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/zuozizuozi/p-zaovra).
:::
