---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

Zaovra bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Zaovra-konfigurasjonen.
...
i `~/.local/share/zaovra/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Zaovra
konfig.
...
Zaovra Zen er en liste over modeller levert av Zaovra-teamet som har vært
testet og verifisert for å fungere godt med Zaovra. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Zaovra Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Zaovra Zen` og gå til [zaovra.com/auth](https://zaovra.com/zen).
...
Det fungerer som alle andre leverandører i Zaovra og er helt valgfritt å bruke.

---

## Zaovra Go

Zaovra Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Zaovra-teamet som har vært
testet og verifisert for å fungere godt med Zaovra.

1. Kjør kommandoen `/connect` i TUI, velg `Zaovra Go`, og gå til [zaovra.com/auth](https://zaovra.com/zen).
...
Slik bruker du Amazon Bedrock med Zaovra:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Zaovra:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `zaovra.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i zaovra støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere zaovra til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Zaovra skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Zaovra-konfigurasjonen.
...
for å lagre token i Zaovra auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Zaovra bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Zaovra
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`zaovra.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `zaovra.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Zaovra:
...
Slik bruker du Google Vertex AI med Zaovra:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Zaovra.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Zaovra-samtale oppført som en separat økt.
...
Du kan konfigurere Zaovra for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Zaovra for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Zaovra for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Zaovra. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/zaovra) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Zaovra:
...
3. Kopier API-nøkkelen for bruk i Zaovra.
...
6. **Viktig**: Før du bruker skymodeller i Zaovra, må du hente modellinformasjonen lokalt:
...
Zaovra Zen er en liste over testede og verifiserte modeller levert av Zaovra-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Zaovra Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Zaovra Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Zaovra API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Zaovra-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Zaovra-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Zaovra:
...
Du kan også tilpasse modeller gjennom Zaovra-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Zaovra. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `zaovra.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Zaovra forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `zaovra auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Zaovra-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Zaovra-konfigurasjonen.
