---
title: Tjener
description: Samhandle med Zaovra-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `zaovra serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Zaovra-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `zaovra.local` |
...
Sett `ZAOVRA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `zaovra` som standard, eller sett `ZAOVRA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `zaovra serve` og `zaovra web`.
...
Når du kjører `zaovra` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Zaovra-serveren til å samhandle med Zaovra programmatisk.
:::

Denne arkitekturen lar Zaovra støtte flere klienter og lar deg samhandle med Zaovra programmatisk.

Du kan kjøre `zaovra serve` for å starte en frittstående server. Hvis du har
Zaovra TUI kjører, vil `zaovra serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Zaovra [IDE](/docs/ide) plugins.
...
zaovra-serveren viser følgende APIer.
