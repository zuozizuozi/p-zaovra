---
title: Server
description: Interagisci con il server zaovra via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Il comando `zaovra serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client zaovra.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `zaovra.local` |
...
Imposta `ZAOVRA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `zaovra`, oppure imposta `ZAOVRA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `zaovra serve` sia per `zaovra web`.
...
Quando esegui `zaovra` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server zaovra per interagire con zaovra in modo programmatico.
:::

Questa architettura permette a zaovra di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `zaovra serve` per avviare un server standalone. Se la TUI di zaovra e gia in esecuzione, `zaovra serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Zaovra.
...
Il server zaovra espone le seguenti API.
