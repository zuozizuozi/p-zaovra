---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Zaovra.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zaovra fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Zaovra avvia la TUI per la directory corrente.
...
Quando usi la TUI di Zaovra, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Zaovra. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `zaovra.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Zaovra attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `zaovra.default`.
