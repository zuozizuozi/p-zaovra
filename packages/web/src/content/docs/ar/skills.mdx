...
تتيح مهارات الوكيل لـ Zaovra اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Zaovra في هذه المواقع:

- إعدادات المشروع: `.zaovra/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/zaovra/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Zaovra صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.zaovra/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/zaovra/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.zaovra/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Zaovra المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `zaovra.json`:
...
**للوكلاء المدمجين** (في `zaovra.json`):
