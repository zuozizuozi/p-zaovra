...
يمكنك إضافة أدوات خارجية إلى Zaovra باستخدام _Model Context Protocol_، أو MCP. يدعم Zaovra خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Zaovra](https://zaovra.com/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/zaovra` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Zaovra تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Zaovra بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Zaovra المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `zaovra mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Zaovra الرموز بشكل آمن في `~/.local/share/zaovra/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Zaovra إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Zaovra مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Zaovra بحساب Sentry الخاص بك.
