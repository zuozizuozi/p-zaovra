...
مع Zaovra يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Zaovra سمتنا `zaovra`.
...
- يريدون أن يطابق Zaovra مظهر terminal لديهم
...
يدعم Zaovra نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/zaovra/themes/*.json` أو `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.zaovra/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.zaovra/themes/*.json`
