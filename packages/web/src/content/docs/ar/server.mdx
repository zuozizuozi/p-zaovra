---
title: الخادم
description: تفاعل مع خادم zaovra عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `zaovra serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل zaovra استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `zaovra.local` |
...
عيّن `ZAOVRA_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `zaovra`، أو عيّن `ZAOVRA_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `zaovra serve` و `zaovra web`.
...
عند تشغيل `zaovra` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم zaovra للتفاعل مع zaovra برمجيا.
:::

تتيح هذه البنية لـ zaovra دعم عدة عملاء وتمكّنك من التفاعل مع zaovra برمجيا.

يمكنك تشغيل `zaovra serve` لبدء خادم مستقل. إذا كانت واجهة zaovra في terminal (TUI)
قيد التشغيل، فسيبدأ `zaovra serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Zaovra لـ [IDE](/docs/ide).
...
يعرض خادم zaovra واجهات API التالية.
