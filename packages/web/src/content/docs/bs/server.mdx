---
title: Server
description: Komunicirajte s zaovra serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `zaovra serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji zaovra klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `zaovra.local` |
...
Postavite `ZAOVRA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `zaovra`, ili postavite `ZAOVRA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `zaovra serve` i za `zaovra web`.
...
Kada pokrenete `zaovra`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite zaovra server za programsku interakciju sa zaovra-om.
:::

Ova arhitektura omogucava zaovra podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `zaovra serve` da startate standalone server. Ako je zaovra TUI vec pokrenut, `zaovra serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Zaovra [IDE](/docs/ide) pluginovi.
...
zaovra server izlaže sljedece API-je.
