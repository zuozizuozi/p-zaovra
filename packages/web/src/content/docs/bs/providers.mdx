---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

Zaovra koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Zaovra konfiguraciji.
...
u `~/.local/share/zaovra/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Zaovra
config.
...
Zaovra Zen je lista modela koje je obezbedio Zaovra tim koji su bili
testirano i potvrđeno da dobro radi sa Zaovra. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Zaovra Zen.
:::
...
Radi kao i svaki drugi provajder u Zaovra i potpuno je opcionalan za korištenje.

---

## Zaovra Go

Zaovra Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Zaovra tim i koji su testirani i verificirani da dobro rade s Zaovra-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Zaovra Go` i idite na [zaovra.com/auth](https://zaovra.com/zen).
...
Da biste koristili Amazon Bedrock s Zaovra:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete zaovra:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `zaovra.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Zaovra.
:::
...
Možete konfigurirati zaovra za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi zaovra ispravno radio.
   :::
...
Također možete dodati modele kroz svoju zaovra konfiguraciju.
...
da pohrani token u zaovra auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Zaovra koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Zaovra
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`zaovra.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth).

##### Konfiguracija

Prilagodite putem `zaovra.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s zaovra:
...
Za korištenje Google Vertex AI s Zaovra:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće zaovra.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem zaovraa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session) da automatski prijavite svaki Zaovra razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Zaovra razgovor naveden kao zasebna sesija.
...
Možete konfigurirati zaovra za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati zaovra za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati zaovra za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Zaovra. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/zaovra) za detalje.
:::
...
Da biste koristili Ollama Cloud s Zaovra:
...
3. Kopirajte API ključ za korištenje u Zaovra.
...
6. **Važno**: Prije upotrebe modela oblaka u Zaovra, morate lokalno povući informacije o modelu:
...
Zaovra Zen je lista testiranih i verifikovanih modela koju je obezbedio Zaovra tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Zaovra Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Zaovra Zen**.

```txt
   /connect
```

3. Unesite svoj Zaovra API ključ.
...
Također možete dodati dodatne modele putem vaše zaovra konfiguracije.
...
5. Također ih možete prilagoditi putem vaše zaovra konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Zaovraom:
...
Također možete prilagoditi modele kroz svoju zaovra konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s zaovra-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `zaovra.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Zaovra da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `zaovra auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Zaovra konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj zaovra konfiguraciji.
