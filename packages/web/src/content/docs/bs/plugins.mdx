---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Zaovra.
---

Dodaci vam omogućavaju da proširite Zaovra spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Zaovra.
...
- `.zaovra/plugins/` - Dodaci na nivou projekta
- `~/.config/zaovra/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/zaovra/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/zaovra/zaovra.json`)
2. Konfiguracija projekta (`zaovra.json`)
3. Globalni direktorij dodataka (`~/.config/zaovra/plugins/`)
4. Direktorij dodataka projekta (`.zaovra/plugins/`)
...
Zaovra pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Zaovra SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Zaovra.
...
Spriječite zaovra da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u zaovra:
...
Pomoćnik `tool` kreira prilagođeni alat koji zaovra može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za zaovra zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://zaovra.com/docs/sdk) za detalje.
