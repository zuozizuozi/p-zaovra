---
title: Konfiguracija
description: Korištenje Zaovra JSON konfiguracije.
---

Možete konfigurirati Zaovra koristeći JSON konfiguracijski fajl.

---

## Format

Zaovra podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/zaovra`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/zaovra/zaovra.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`ZAOVRA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`zaovra.json` u projektu) - postavke specifične za projekat
5. **`.zaovra` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`ZAOVRA_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.zaovra` i `~/.config/zaovra` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/zaovra` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Zaovra konfiguraciju u `~/.config/zaovra/zaovra.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/zaovra/tui.json`.
...
Dodajte `zaovra.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Zaovra pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `ZAOVRA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `ZAOVRA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.zaovra` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.zaovra`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`zaovra.com/config.json`**](https://zaovra.com/config.json).

TUI konfiguracija koristi [**`zaovra.com/tui.json`**](https://zaovra.com/tui.json).
...
Koristite `ZAOVRA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `zaovra.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `zaovra serve` i `zaovra web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Zaovra server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `zaovra.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Zaovra konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Zaovra pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/zaovra/agents/` ili `.zaovra/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Zaovra će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`zaovra run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/zaovra/commands/` ili `.zaovra/commands/`. [Saznajte više ovdje](/docs/commands).
...
Zaovra će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Zaovra **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Zaovra sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.zaovra/plugins/` ili `~/.config/zaovra/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Zaovra da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
