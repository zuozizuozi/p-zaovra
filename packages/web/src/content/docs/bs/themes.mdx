...
U Zaovra mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Zaovra koristi nasu `zaovra` temu.
...
Zaovra dolazi sa vise ugradenih tema.
...
- Zele da Zaovra odgovara izgledu njihovog terminala
...
Zaovra podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/zaovra/themes/*.json` ili `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.zaovra/themes/*.json`
4. **Trenutni radni direktorij** - `./.zaovra/themes/*.json`
