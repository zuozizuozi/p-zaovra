...
Create `.zaovra/commands/test.md`:
...
You can add custom commands through the Zaovra config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Zaovra [config](/docs/config):
...
zaovra includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
