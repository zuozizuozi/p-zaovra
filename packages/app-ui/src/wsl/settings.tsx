...
import { wslZaovraAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.zaovraChecks[item.config.distro]
          const zaovraAction = () => wslZaovraAction(check())
          const busy = () => wsl.data?.job?.kind === "install-zaovra" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={zaovraAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installZaovra(item.config.distro))}
                    >
