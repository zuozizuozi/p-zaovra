...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "zaovra-go") return language.t("dialog.provider.zaovraGo.tagline")
    return undefined
  }
...
function ProviderConnection(props: {
  ...
  const attempt = { id: undefined as string | undefined, complete: false }
  ...
  onCleanup(() => {
    ...
    if (!attempt.id || attempt.complete) return
    const location = auth.latest?.location
    void serverSDK()
      .client.v2.integration.attempt.cancel({ attemptID: attempt.id, location }, { throwOnError: true })
      .catch(() => undefined)
  })
  ...
  const methods = createMemo<ConnectMethod[]>(() =>
    (auth.latest?.integration?.methods ?? []).filter(
      (method): method is ConnectMethod => method.type === "key" || method.type === "oauth",
    ),
  )
  const [store, setStore] = createStore({
    methodIndex: undefined as undefined | number,
    authorization: undefined as undefined | IntegrationAttempt,
  ...
  type Action =
    ...
    | { type: "auth.pending" }
    | { type: "auth.complete"; authorization: IntegrationAttempt }
  ...
  async function selectMethod(index: number, inputs?: Record<string, string>) {
    ...
    if (!method) return
    if (attempt.id && !attempt.complete) {
      void serverSDK()
        .client.v2.integration.attempt.cancel(
          { attemptID: attempt.id, location: auth.latest?.location },
          { throwOnError: true },
        )
        .catch(() => undefined)
    ...
    if (method.type === "key" && method.prompts?.length) {
      if (!inputs) {
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          const authorization = x.data.data
          attempt.id = authorization.attemptID
  ...
  function AuthPromptsView() {
    ...
    const prompts = createMemo<NonNullable<ConnectMethod["prompts"]>>(() => {
      const value = method()
  ...
  createEffect(() => {
    ...
    if (methods().length === 0) {
      auto = true
      dispatch({ type: "auth.error", error: `Provider ${props.provider} has no supported authentication method` })
      return
    }
    if (methods().length === 1) {
      auto = true
      ...
      <>
        ...
        <div>
          <List
            ...
            items={methods}
            key={(m) => (m?.type === "oauth" ? m.id : (m?.label ?? m?.type ?? ""))}
  ...
  function ApiAuthView() {
    ...
    async function handleSubmit(e: SubmitEvent) {
      ...
      const key = formStore.value.trim()
      if (!key) {
        setFormStore("error", language.t("provider.connect.apiKey.required"))
        return
      }
      setFormStore({ pending: true, error: undefined })
      try {
        ...
        if (alive.value) setFormStore("pending", false)
      }
    }
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "zaovra"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.zaovraZen.line1")}</div>
              <div>{language.t("provider.connect.zaovraZen.line2")}</div>
              <div>
          ...
          <form onSubmit={handleSubmit} class="flex flex-col items-start gap-5 self-stretch">
            ...
            <ButtonV2 type="submit" variant="contrast" disabled={formStore.pending}>
              {formStore.pending ? language.t("provider.custom.discovery.loading") : language.t("common.continue")}
            </ButtonV2>
          </form>
        </div>
      )
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "zaovra"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.zaovraZen.line1")}</div>
  ...
  function OAuthCodeView() {
    ...
    async function handleSubmit(e: SubmitEvent) {
      ...
      const result = await serverSDK()
        .client.v2.integration.attempt.complete(
          { attemptID: store.authorization!.attemptID, location: auth.latest!.location, code },
          { throwOnError: true },
        )
  ...
  function OAuthAutoView() {
    ...
    onMount(() => {
      void (async () => {
        while (alive.value) {
          const result = await serverSDK()
            .client.v2.integration.attempt.status(
              { attemptID: store.authorization!.attemptID, location: auth.latest!.location },
              { throwOnError: true },
            )
            .then((response) => ({ ok: true as const, status: response.data.data }))
            .catch((error) => ({ ok: false as const, error }))
          if (!alive.value) return
          if (!result.ok) {
          ...
          if (result.status.status === "expired") {
            dispatch({ type: "auth.error", error: "Authorization expired" })
            return
          }
