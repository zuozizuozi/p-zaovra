import type { Event, Session, SessionV2Info, V2SessionListResponse } from "@zaovra-ai/sdk/v2/client"
...
export function homeSessionIndexRefresh(event: Event["type"], connected: boolean) {
  ...
  return {
    ...
    refetch:
      ...
      event === "session.next.created" ||
      event === "session.next.moved" ||
      event === "session.next.updated" ||
      event === "session.next.deleted",
  }
}
