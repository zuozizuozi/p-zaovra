...
import { SessionStatusEvent } from "@zaovra-ai/schema/session-status-event"
import { SessionV1 } from "@zaovra-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Zaovra/TimelineStability"
...
export const model = { providerID: "zaovra", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("zaovra.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
