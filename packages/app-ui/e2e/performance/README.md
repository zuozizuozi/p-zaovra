...
The app's high-volume performance diagnostics live under `packages/app-ui/e2e/performance` and are excluded from normal local and CI Playwright discovery. The benchmark config builds the app and serves the production bundle before running scenarios serially.

Run the suite explicitly from `packages/app-ui`:
...
Set `ZAOVRA_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
ZAOVRA_PERFORMANCE_TRACE_DIR=/tmp/zaovra-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `ZAOVRA_PERFORMANCE_SELECTOR_TRACE=1`.
