  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://zaovra.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://zaovra.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://zaovra.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://zaovra.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://zaovra.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://zaovra.example.com"),
      ),
    ).toEqual({
