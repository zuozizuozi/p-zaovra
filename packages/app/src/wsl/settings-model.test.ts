...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Zaovra needs attention", () => {
    expect(wslZaovraAction(undefined)).toBeUndefined()
    expect(
      wslZaovraAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    expect(
      wslZaovraAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/zaovra",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Zaovra is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        zaovraChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.zaovra/bin/zaovra",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installZaovra" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-zaovra")
  })
