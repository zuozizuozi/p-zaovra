...
export const ZAOVRA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ZAOVRA_MODE_KEY = "zaovra.mode"

export const ZaovraKeymapProvider = KeymapProvider
export const useZaovraKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ZaovraModeStack = ReturnType<typeof createZaovraModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ZaovraModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createZaovraModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ZAOVRA_MODE_KEY, ZAOVRA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ZAOVRA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ZAOVRA_MODE_KEY, stack.at(-1)?.mode ?? ZAOVRA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ZAOVRA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ZAOVRA_MODE_KEY, undefined)
...
export function useZaovraModeStack() {
  return getZaovraModeStack(useZaovraKeymap())
}

export function getZaovraModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Zaovra mode stack is not registered for this keymap")
  return value
}
...
export function registerZaovraKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createZaovraModeStack(keymap)
