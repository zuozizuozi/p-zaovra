...
import { getZaovraModeStack, ZAOVRA_BASE_MODE, ZaovraKeymapProvider, registerZaovraKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerZaovraKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when zaovra mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getZaovraModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getZaovraModeStack(keymap).push("autocomplete")
