import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@zaovra-ai/sdk/v2"
import { SessionTurn } from "@zaovra-ai/session-ui/session-turn"
import { SessionReview } from "@zaovra-ai/session-ui/session-review"
import { DataProvider } from "@zaovra-ai/session-ui/context"
import { FileComponentProvider } from "@zaovra-ai/ui/context/file"
import { WorkerPoolProvider } from "@zaovra-ai/ui/context/worker-pool"
...
import { iife } from "@zaovra-ai/core/util/iife"
...
import { MessageNav } from "@zaovra-ai/session-ui/message-nav"
import { FileSSR } from "@zaovra-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@zaovra-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/zaovra-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Zaovra</Title>
              </Show>
              <Meta name="description" content="zaovra - The AI coding agent built for the terminal." />
