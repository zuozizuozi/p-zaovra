---
title: Config
description: Using the Zaovra JSON config.
---

You can configure Zaovra using a JSON config file.

---

## Format

Zaovra supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/zaovra`) - organizational defaults
2. **Global config** (`~/.config/zaovra/zaovra.json`) - user preferences
3. **Custom config** (`ZAOVRA_CONFIG` env var) - custom overrides
4. **Project config** (`zaovra.json` in project) - project-specific settings
5. **`.zaovra` directories** - agents, commands, plugins
6. **Inline config** (`ZAOVRA_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/zaovra/` on macOS) - admin-controlled
...
:::note
The `.zaovra` and `~/.config/zaovra` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/zaovra` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Zaovra config in `~/.config/zaovra/zaovra.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/zaovra/tui.json`.
...
Add `zaovra.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Zaovra starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `ZAOVRA_CONFIG` environment variable.
...
Specify a custom config directory using the `ZAOVRA_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.zaovra` directory, and should
...
The custom directory is loaded after the global config and `.zaovra` directories, so it **can override** their settings.
...
Drop an `zaovra.json` or `zaovra.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/zaovra/` |
| Linux    | `/etc/zaovra/`                         |
| Windows  | `%ProgramData%\zaovra`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Zaovra reads managed preferences from the `ai.zaovra.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Zaovra checks these paths:

1. `/Library/Managed Preferences/<user>/ai.zaovra.managed.plist`
2. `/Library/Managed Preferences/ai.zaovra.managed.plist`

The plist keys map directly to `zaovra.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.zaovra.managed` PayloadType. The Zaovra config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.zaovra.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.zaovra.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.zaovra</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`zaovra.com/config.json`**](https://zaovra.com/config.json).

TUI config uses [**`zaovra.com/tui.json`**](https://zaovra.com/tui.json).
...
Use `ZAOVRA_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `zaovra.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `zaovra serve` and `zaovra web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Zaovra server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `zaovra.local`. Useful for running multiple instances on the same network.
...
If not specified, Zaovra will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Zaovra config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Zaovra tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Zaovra actions on configured resources. Currently, policies can control which providers Zaovra may use.
...
Zaovra normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Zaovra omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/zaovra/agents/` or `.zaovra/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Zaovra will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`zaovra run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/zaovra/commands/` or `.zaovra/commands/`. [Learn more here](/docs/commands).
...
Zaovra uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Zaovra will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, zaovra **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Zaovra with custom tools, hooks, and integrations.

Place plugin files in `.zaovra/plugins/` or `~/.config/zaovra/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Zaovra to only use specific providers rather than disabling them one by one.
