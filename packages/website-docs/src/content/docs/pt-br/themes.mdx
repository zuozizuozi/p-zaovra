...
Com o zaovra, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o zaovra usa nosso próprio tema `zaovra`.
...
O zaovra vem com vários temas embutidos.
...
- Querem que o zaovra corresponda à aparência do seu terminal
...
O zaovra suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/zaovra/themes/*.json` ou `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.zaovra/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.zaovra/themes/*.json`
