...
Existem dois tipos de agentes no zaovra; agentes primários e subagentes.
...
zaovra vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Zaovra vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Zaovra vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Zaovra, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `zaovra.json`:
...
- Global: `~/.config/zaovra/agents/`
- Por projeto: `.zaovra/agents/`

```markdown title="~/.config/zaovra/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o zaovra usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do zaovra quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do zaovra usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Zaovra Zen](/docs/zen), você usaria `zaovra/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="zaovra.json" {3-6,9-12}
{
  "$schema": "https://zaovra.com/config.json",
  "tools": {
    "write": true,
...
```json title="zaovra.json" {3-5,8-10}
{
  "$schema": "https://zaovra.com/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `zaovra models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/zuozizuozi/p-zaovra).
:::
...
```markdown title="~/.config/zaovra/agents/docs-writer.md"
---
...
```markdown title="~/.config/zaovra/agents/security-auditor.md"
---
