---
title: Plugins
description: Escreva seus próprios plugins para estender o zaovra.
---

Plugins permitem que você estenda o zaovra conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do zaovra.
...
- `.zaovra/plugins/` - Plugins em nível de projeto
- `~/.config/zaovra/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/zaovra/node_modules/`.
...
1. Configuração global (`~/.config/zaovra/zaovra.json`)
2. Configuração do projeto (`zaovra.json`)
3. Diretório de plugins global (`~/.config/zaovra/plugins/`)
4. Diretório de plugins do projeto (`.zaovra/plugins/`)
...
O zaovra executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do zaovra para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o zaovra.
...
:::note
Se você estiver usando o aplicativo desktop zaovra, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o zaovra de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao zaovra:
...
O helper `tool` cria uma ferramenta personalizada que o zaovra pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o zaovra junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://zaovra.com/docs/sdk) para detalhes.
