...
As habilidades do agente permitem que o zaovra descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O zaovra pesquisa nesses locais:

- Configuração do projeto: `.zaovra/skills/<name>/SKILL.md`
- Configuração global: `~/.config/zaovra/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o zaovra sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.zaovra/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.zaovra/skills/git-release/SKILL.md` assim:
...
O zaovra lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `zaovra.json`:
...
**Para agentes embutidos** (em `zaovra.json`):
