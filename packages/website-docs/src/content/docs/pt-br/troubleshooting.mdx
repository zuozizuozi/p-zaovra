...
Para depurar problemas com o zaovra, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\zaovra\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `zaovra --log-level DEBUG`.

---

## Armazenamento

O zaovra armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/zaovra/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\zaovra`
...
O zaovra Desktop executa um servidor zaovra local (o sidecar `zaovra-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Zaovra` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/zaovra/zaovra.jsonc` (ou `~/.config/zaovra/zaovra.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/zaovra/zaovra.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\zaovra\zaovra.jsonc`
...
O zaovra também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/zaovra/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\zaovra\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o zaovra possa reconstruí-lo.

1. Saia completamente do zaovra Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/zaovra`
- **Linux**: exclua `~/.cache/zaovra` (ou execute `rm -rf ~/.cache/zaovra`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\zaovra`

3. Reinicie o zaovra Desktop.

---

### Corrigindo problemas de conexão com o servidor

O zaovra Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `zaovra.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `ZAOVRA_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `ZAOVRA_PORT` (ou escolha uma porta livre) e reinicie.
...
- Se você estiver no Wayland e o aplicativo estiver em branco/travando, tente iniciar com `ZAOVRA_ALLOW_WAYLAND=1`.
...
No Windows, o zaovra Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do zaovra.

---

### Notificações não aparecendo

O zaovra Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o zaovra nas configurações do seu sistema operacional, e
...
1. Saia do zaovra Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo zaovra Desktop):

- `zaovra.settings.dat` (URL do servidor padrão do desktop)
- `zaovra.global.dat` e `zaovra.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o zaovra:
...
3. Certifique-se de que você tem a versão mais recente com `zaovra upgrade`
...
Para descobrir quais modelos você tem acesso, execute `zaovra models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\zaovra`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O zaovra instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\zaovra`

2. Reinicie o zaovra para reinstalar os pacotes de provedores mais recentes

Isso forçará o zaovra a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O zaovra detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
