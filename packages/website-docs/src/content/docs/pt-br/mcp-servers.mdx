...
Você pode adicionar ferramentas externas ao zaovra usando o _Model Context Protocol_, ou MCP. O zaovra suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do zaovra](https://zaovra.com/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/zaovra`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O zaovra lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o zaovra irá:
...
Se o servidor requer autenticação, o zaovra solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `zaovra mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o zaovra armazenará os tokens de forma segura em `~/.local/share/zaovra/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no zaovra, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do zaovra como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o zaovra à sua conta do Sentry.
