...
description: Skriv dine egne programtillegg for å utvide Zaovra.
---

Plugins lar deg utvide Zaovra ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Zaovra.
...
- `.zaovra/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/zaovra/node_modules/`.
...
1. Global konfigurasjon (`~/.config/zaovra/zaovra.json`)
2. Prosjektkonfigurasjon (`zaovra.json`)
3. Global plugin-katalog (`~/.config/zaovra/plugins/`)
4. Prosjektpluginkatalog (`.zaovra/plugins/`)
...
Zaovra kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Zaovra SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Zaovra.
...
:::note
Hvis du bruker Zaovra-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Zaovra fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Zaovra:
...
`tool`-hjelperen lager et tilpasset verktøy som Zaovra kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Zaovra sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://zaovra.com/docs/sdk) for detaljer.
