...
_Model Bağlam Protokolü_ veya MCP kullanarak zaovra'a harici araçlar ekleyebilirsiniz. zaovra hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [zaovra Config](https://zaovra.com/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/zaovra` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
zaovra, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde zaovra şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa zaovra, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `zaovra mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından zaovra, belirteçleri `~/.local/share/zaovra/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra zaovra'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi zaovra yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve zaovra'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
