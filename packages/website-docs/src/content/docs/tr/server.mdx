---
title: Sunucu
description: zaovra sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

`zaovra serve` komutu, zaovra istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `zaovra.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ZAOVRA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `zaovra` değeridir; değiştirmek isterseniz `ZAOVRA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `zaovra serve` hem de `zaovra web` için geçerlidir.
...
`zaovra` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
zaovra ile programatik etkileşim için zaovra sunucusunu kullanın.
:::

Bu mimari, zaovra'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`zaovra serve` ile bağımsız bir sunucu başlatabilirsiniz. zaovra TUI açıksa, `zaovra serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum zaovra [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
zaovra sunucusu aşağıdaki API'leri sunar.
