---
title: Sağlayıcılar
description: zaovra'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

zaovra, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı zaovra yapılandırmanızda yapılandırın.
...
`~/.local/share/zaovra/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları zaovra'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Zaovra Zen, zaovra ekibi tarafından sağlanan modellerin bir listesidir.
zaovra ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Zaovra Zen ile başlamanızı öneririz.
:::
...
zaovra'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Zaovra Go

Zaovra Go, zaovra ile iyi çalıştığı test edilmiş ve doğrulanmış, zaovra ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Zaovra Go`'yu seçin ve [zaovra.com/auth](https://zaovra.com/zen) adresine gidin.
...
Amazon Bedrock'u zaovra ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   zaovra'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `zaovra.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi zaovra'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
zaovra'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   zaovra'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Zaovra yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
zaovra, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. zaovra'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`zaovra.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth) ana sayfasında.

##### Yapılandırma

`zaovra.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi zaovra'la kullanmak için:
...
Google Vertex AI'yi zaovra ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları zaovra çalıştırılırken ayarlayın.
...
Helicone'un zaovra aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her zaovra görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her zaovra görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
zaovra'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla zaovra'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla zaovra'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini zaovra için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/zaovra)'a bakın.
:::
...
Ollama Cloud'u zaovra ile kullanmak için:
...
3. zaovra'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: zaovra'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Zaovra Zen, zaovra ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Zaovra Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Zaovra Zen**'i arayın.

   ```txt
   /connect
   ```

3. zaovra API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   zaovra yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca zaovra yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Zaovra ile kullanmak için:
...
Modelleri zaovra yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı zaovra ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `zaovra.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları zaovra'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `zaovra auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için zaovra yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, zaovra yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
