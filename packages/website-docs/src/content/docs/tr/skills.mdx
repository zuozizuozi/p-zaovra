...
Ajan becerileri, zaovra'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
zaovra su konumlari tarar:

- Proje konfigurasyonu: `.zaovra/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/zaovra/skills/<name>/SKILL.md`
...
Proje yerel yollarinda zaovra, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.zaovra/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.zaovra/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
zaovra, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`zaovra.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`zaovra.json` icinde):
