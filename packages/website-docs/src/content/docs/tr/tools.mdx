...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. zaovra yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
zaovra içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `ZAOVRA_EXPERIMENTAL_LSP_TOOL=true` (veya `ZAOVRA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca zaovra provider kullanılırken veya `ZAOVRA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

zaovra başlatırken etkinleştirmek için:
