...
أنشئ الملف `.zaovra/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Zaovra أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Zaovra:
...
- على مستوى النظام: `~/.config/zaovra/commands/`
- لكل مشروع: `.zaovra/commands/`
...
يتضمن zaovra عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
