---
title: المزوّدون
description: استخدام أي موفّر LLM في Zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Zaovra ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Zaovra.
...
في `~/.local/share/zaovra/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Zaovra.
...
Zaovra Zen هي قائمة نماذج يوفّرها فريق Zaovra وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Zaovra. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Zaovra Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر zaovra، ثم انتقل إلى [zaovra.com/auth](https://zaovra.com/auth).
...
يعمل مثل أي مزوّد آخر في Zaovra واستخدامه اختياري بالكامل.

---

## Zaovra Go

Zaovra Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Zaovra، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Zaovra.

1. شغّل الأمر `/connect` في TUI، واختر `Zaovra Go`، ثم انتقل إلى [zaovra.com/auth](https://zaovra.com/zen).
...
لاستخدام Amazon Bedrock مع Zaovra:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل zaovra:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `zaovra.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Zaovra غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين zaovra لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل zaovra بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات zaovra.
...
تخزين الرمز في مساحة تخزين مصادقة zaovra.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Zaovra نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Zaovra
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`zaovra.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth).

##### التهيئة

خصّص عبر `zaovra.json`:
...
لاستخدام اشتراك GitHub Copilot مع zaovra:
...
لاستخدام Google Vertex AI مع Zaovra:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر zaovra، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session) لتسجيل كل محادثة Zaovra تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Zaovra مدرجة كجلسة مستقلة.
...
يمكنك ضبط zaovra لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط zaovra لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط zaovra لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Zaovra. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/zaovra) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Zaovra:
...
3. انسخ مفتاح API لاستخدامه في Zaovra.
...
6. **مهم**: قبل استخدام نماذج السحابة في Zaovra، يجب سحب معلومات النموذج محليا:
...
Zaovra Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Zaovra. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Zaovra Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Zaovra Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Zaovra.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات zaovra.
...
5. يمكنك أيضا تخصيصها عبر إعدادات zaovra. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Zaovra:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات zaovra. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع zaovra. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `zaovra.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Zaovra بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `zaovra auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات zaovra وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات zaovra.
