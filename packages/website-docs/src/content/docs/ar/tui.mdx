---
title: TUI
description: استخدام واجهة المستخدم TUI في Zaovra.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Zaovra واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Zaovra إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Zaovra في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Zaovra. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Zaovra. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `zaovra.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Zaovra بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `zaovra.default`.
