...
Zaovra has a list of keybinds that you can customize through `tui.json`.
...
Zaovra uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long Zaovra waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The Zaovra desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `zaovra.json`.
