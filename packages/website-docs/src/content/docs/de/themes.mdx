...
Mit Zaovra kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Zaovra das `zaovra`-Theme.
...
Zaovra liefert mehrere eingebaute Themes mit.
...
- Zaovra optisch ans Terminal anpassen wollen
...
Zaovra unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/zaovra/themes/*.json` oder `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.zaovra/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.zaovra/themes/*.json`
