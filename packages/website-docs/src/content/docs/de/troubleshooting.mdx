...
Wenn Zaovra Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zaovra\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `zaovra --log-level DEBUG`.

---

## Speicher

zaovra speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/zaovra/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zaovra`
...
Zaovra Desktop startet im Hintergrund einen lokalen Zaovra-Server (`zaovra-cli`-Sidecar).
...
- Nur macOS: `Zaovra`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/zaovra/zaovra.jsonc` (oder `~/.config/zaovra/zaovra.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/zaovra/zaovra.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zaovra\zaovra.jsonc`
...
Zaovra kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/zaovra/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zaovra\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Zaovra ihn neu aufbauen kann.

1. Beende Zaovra Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/zaovra`
- **Linux**: loesche `~/.cache/zaovra` (oder fuehre aus: `rm -rf ~/.cache/zaovra`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\zaovra`

3. Starte Zaovra Desktop neu.

---

### Server-Verbindungsprobleme beheben

Zaovra Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `zaovra.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `ZAOVRA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `ZAOVRA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
- Wenn du Wayland nutzt und die App leer ist/abstuerzt, versuche den Start mit `ZAOVRA_ALLOW_WAYLAND=1`.
...
Unter Windows benoetigt Zaovra Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Zaovra-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Zaovra Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Zaovra in den OS-Einstellungen aktiviert sind, und
...
1. Beende Zaovra Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Zaovra Desktop):

- `zaovra.settings.dat` (desktop default server URL)
- `zaovra.global.dat` und `zaovra.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Zaovra hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `zaovra models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\zaovra`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. zaovra installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\zaovra`

2. Starte zaovra neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt zaovra, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
zaovra erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
