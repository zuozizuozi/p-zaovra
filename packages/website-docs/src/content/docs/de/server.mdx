---
title: Server
description: Nutze den zaovra-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

Der Befehl `zaovra serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein zaovra-Client nutzen kann.
...
Setze `ZAOVRA_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `zaovra`, kann aber mit `ZAOVRA_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `zaovra serve` und `zaovra web`.
...
Wenn du `zaovra` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den zaovra-Server, um zaovra programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `zaovra serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `zaovra serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Zaovra-[IDE](/docs/ide)-Plugins.
...
Der zaovra-Server stellt folgende APIs bereit.
