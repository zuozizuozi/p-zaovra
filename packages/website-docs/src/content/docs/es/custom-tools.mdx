...
description: Cree herramientas que LLM pueda llamar en zaovra.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de zaovra como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.zaovra/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/zaovra/tools/`.
...
```ts title=".zaovra/tools/database.ts" {1}
...
```ts title=".zaovra/tools/project.ts" {8}
import { tool } from "@zaovra-ai/plugin"
...
```python title=".zaovra/tools/add.py"
import sys
...
```ts title=".zaovra/tools/python-add.ts" {10}
import { tool } from "@zaovra-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".zaovra/tools/add.py")
