...
Las habilidades del agente permiten a Zaovra descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Zaovra busca estas ubicaciones:

- Configuración del proyecto: `.zaovra/skills/<name>/SKILL.md`
- Configuración global: `~/.config/zaovra/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Zaovra sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.zaovra/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.zaovra/skills/git-release/SKILL.md` así:
...
Zaovra enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `zaovra.json`:
...
**Para agentes integrados** (en `zaovra.json`):
