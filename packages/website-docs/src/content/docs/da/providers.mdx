---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

Zaovra bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Zaovra-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/zaovra/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Zaovra-konfigurationen.
...
Zaovra Zen er en liste over modeller leveret af Zaovra-teamet, som er blevet testet og verificeret til at fungere godt med Zaovra. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Zaovra Zen.
:::
...
Det fungerer som alle andre udbydere i Zaovra og er helt valgfrit at bruge.

---

## Zaovra Go

Zaovra Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Zaovra-teamet, som er testet og verificeret til at fungere godt med Zaovra.

1. Kør kommandoen `/connect` i TUI, vælg `Zaovra Go`, og gå til [zaovra.com/auth](https://zaovra.com/zen).
...
Sådan bruger du Amazon Bedrock med Zaovra:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører zaovra:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `zaovra.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Zaovra understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere zaovra til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at zaovra skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem zaovra-konfigurationen.
...
gemme tokenet i zaovra auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Zaovra bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Zaovra
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`zaovra.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `zaovra.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Zaovra:
...
Sådan bruger du Google Vertex AI med Zaovra:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem zaovra, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session) plugin for automatisk at logge hver Zaovra-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Zaovra-samtale opført som en separat session.
...
Du kan konfigurere zaovra til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere zaovra til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere zaovra til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Zaovra. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/zaovra) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Zaovra:
...
3. Kopier API-nøglen til brug i Zaovra.
...
6. **Vigtigt**: Før du bruger skymodeller i Zaovra, skal du hente modelinformationen lokalt:
...
Zaovra Zen er en liste over testede og verificerede modeller leveret af Zaovra-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Zaovra Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Zaovra Zen**.

   ```txt
   /connect
   ```

3. Indtast Zaovra API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem zaovra-konfigurationen.
...
5. Du kan også tilpasse dem gennem zaovra-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Zaovra:
...
Du kan også tilpasse modeller gennem zaovra-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med zaovra. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `zaovra.json`-filen i projektkataloget:
...
`limit`-felterne lader Zaovra forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `zaovra auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek zaovra-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i zaovra-konfigurationen.
