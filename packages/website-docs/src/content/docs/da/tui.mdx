---
title: TUI
description: Brug af Zaovra-terminalbrugergrænsefladen.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zaovra giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Zaovra starter TUI for den nuværende mappe.
...
Når du bruger Zaovra TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Zaovra. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Afslut Zaovra. _Aliaser_: `/quit`, `/q`
...
Dette er adskilt fra `zaovra.json`, som konfigurerer server-/kørselstidsadfærd.
...
- `leader_timeout` - Styrer hvor længe Zaovra venter efter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `zaovra.default`.
