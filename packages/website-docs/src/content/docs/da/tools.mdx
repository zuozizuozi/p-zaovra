...
Her er alle de innebygde verktøyene som er tilgængelige i Zaovra.
...
:::note
Dette verktøyet er bare tilgængelig når `ZAOVRA_EXPERIMENTAL_LSP_TOOL=true` (eller `ZAOVRA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Zaovra-leverandøren eller når miljøvariabelen `ZAOVRA_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
