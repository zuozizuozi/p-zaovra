---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć zaovra.
---

Wtyczki umożliwiające wyłączenie zaovra poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania zaovra.
...
- `.zaovra/plugins/` - Wtyczki na poziomie projektu
- `~/.config/zaovra/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/zaovra/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/zaovra/zaovra.json`)
2. Konfiguracja projektu (`zaovra.json`)
3. Globalny katalog wtyczek (`~/.config/zaovra/plugins/`)
4. Katalog wtyczek projektu (`.zaovra/plugins/`)
...
zaovra uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z zaovra do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do zaovra.
...
:::note
Jeśli korzystasz z aplikacji komputerowej zaovra, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj zaovra czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do zaovra:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod zaovra. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://zaovra.com/docs/sdk).
