---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w zaovra.
---

import config from "../../../../config.mjs"
export const console = config.console

zaovra używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji zaovra.
...
w `~/.local/share/zaovra/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim zaovra
config.
...
Zaovra Zen to lista modeli dostarczonych przez zespół zaovra, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z zaovra. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Zaovra Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Zaovra Zen` i przejdź do [zaovra.com/auth](https://zaovra.com/zen).
...
Działa jak każdy inny dostawca w zaovra i jest całkowicie opcjonalny w użyciu.

---

## Zaovra Go

Zaovra Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół zaovra, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z zaovra.

1. Uruchom polecenie `/connect` w TUI, wybierz `Zaovra Go` i przejdź do [zaovra.com/auth](https://zaovra.com/zen).
...
Aby używać Amazon Bedrock z zaovra:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania zaovra:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `zaovra.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w zaovra nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować zaovra do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod zaovra działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji zaovra.
...
zaovra używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować zaovra
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`zaovra.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [zaovra-gitlab-auth](https://www.npmjs.com/package/zaovra-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem zaovra:
...
Aby używać Google Vertex AI z zaovra:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania zaovra.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą zaovra, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [zaovra-helicone-session](https://github.com/H2Shami/zaovra-helicone-session), aby automatycznie rejestrować każdą konwersację zaovra jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja zaovra będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować zaovra tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować zaovra tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować zaovra tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem zaovra. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/zaovra).
:::
...
Aby korzystać z Ollama Cloud z zaovra:
...
3. Skopiuj klucz API do użycia w zaovra.
...
6. **Ważne**: Przed użyciem modeli chmurowych w zaovra musisz pobrać informacje o modelu lokalnie:
...
Zaovra Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół zaovra. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Zaovra Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Zaovra Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API zaovra.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji zaovra.
...
5. Możesz je także dostosować za pomocą konfiguracji zaovra. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z zaovra:
...
Możesz także dostosować modele za pomocą konfiguracji zaovra. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z zaovra. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `zaovra.json` w katalogu projektu:
...
Pola `limit` pozwalają zaovra zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `zaovra auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację zaovra i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji zaovra.
