...
Dzięki zaovra możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie zaovra używa naszego własnego motywu `zaovra`.
...
zaovra ma kilka wbudowanych motywów.
...
- Chcą, aby Zaovra pasował do wyglądu ich terminala
...
zaovra obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/zaovra/themes/*.json` lub `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.zaovra/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.zaovra/themes/*.json`
