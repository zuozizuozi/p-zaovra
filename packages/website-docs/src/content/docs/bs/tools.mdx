...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Zaovra dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Zaovra.
...
:::note
Ovaj alat je dostupan samo kada je `ZAOVRA_EXPERIMENTAL_LSP_TOOL=true` (ili `ZAOVRA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Zaovra provajdera ili kada je varijabla `ZAOVRA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Zaovra:
