...
Agent skills omogucavaju Zaovra da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Zaovra pretrazuje ove lokacije:

- Konfiguracija projekta: `.zaovra/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/zaovra/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Zaovra ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.zaovra/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.zaovra/skills/git-release/SKILL.md` ovako:
...
Zaovra navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `zaovra.json`:
...
**Za ugradene agente** (u `zaovra.json`):
