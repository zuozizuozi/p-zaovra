---
title: LSP serveri
description: Zaovra se integriše sa vašim LSP serverima.
---

Zaovra se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Zaovra dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `ZAOVRA_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i zaovra otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj zaovra konfiguraciji.
