...
Možete dodati vanjske alate u Zaovra koristeći _Model Context Protocol_, ili MCP. Zaovra podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Zaovra Config](https://zaovra.com/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/zaovra`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Zaovra automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Zaovra će:
...
Ako server zahtijeva autentifikaciju, Zaovra će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `zaovra mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Zaovra sigurno čuva tokene u `~/.local/share/zaovra/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Zaovra, zajedno s ugrađenim alatima. Možete njima upravljati kroz Zaovra konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Zaovra sa vašim Sentry nalogom.
