---
title: Serveur
description: Interagissez avec le serveur zaovra via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

La commande `zaovra serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client zaovra peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `zaovra.local` |
...
Définissez `ZAOVRA_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `zaovra`, ou définissez `ZAOVRA_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `zaovra serve` et à `zaovra web`.
...
Lorsque vous exécutez `zaovra`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur zaovra pour interagir avec zaovra par programmation.
:::

Cette architecture permet à zaovra de prendre en charge plusieurs clients et vous permet d'interagir avec zaovra par programme.

Vous pouvez exécuter `zaovra serve` pour démarrer un serveur autonome. Si vous avez le
zaovra TUI en cours d'exécution, `zaovra serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Zaovra [IDE](/docs/ide).
...
Le serveur zaovra expose les API suivantes.
