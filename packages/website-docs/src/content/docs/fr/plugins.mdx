---
title: Plugins
description: Écrivez vos propres plugins pour étendre Zaovra.
---

Les plugins vous permettent d'étendre Zaovra en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Zaovra.
...
- `.zaovra/plugins/` - Plugins au niveau du projet
- `~/.config/zaovra/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/zaovra/node_modules/`.
...
1. Configuration globale (`~/.config/zaovra/zaovra.json`)
2. Configuration du projet (`zaovra.json`)
3. Répertoire global des plugins (`~/.config/zaovra/plugins/`)
4. Répertoire des plugins du projet (`.zaovra/plugins/`)
...
Zaovra exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK zaovra pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre zaovra.
...
:::note
Si vous utilisez l'application de bureau Zaovra, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez zaovra de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à zaovra :
...
L'assistant `tool` crée un outil personnalisé que zaovra peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour zaovra aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://zaovra.com/docs/sdk) pour plus de détails.
