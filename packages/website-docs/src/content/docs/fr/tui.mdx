---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Zaovra.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zaovra fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Zaovra démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Zaovra TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Zaovra. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `zaovra.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Zaovra attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `zaovra.default`.
