...
Tools allow the LLM to perform actions in your codebase. Zaovra comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Zaovra.
...
:::note
This tool is only available when `ZAOVRA_EXPERIMENTAL_LSP_TOOL=true` (or `ZAOVRA_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Zaovra provider or when the `ZAOVRA_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Zaovra:
