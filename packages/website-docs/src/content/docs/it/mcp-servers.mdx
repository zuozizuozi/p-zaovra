...
Puoi aggiungere strumenti esterni a Zaovra usando il _Model Context Protocol_, o MCP. Zaovra supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Zaovra Config](https://zaovra.com/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/zaovra`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Zaovra gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Zaovra:
...
Se il server richiede autenticazione, Zaovra ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `zaovra mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Zaovra memorizzera' i token in modo sicuro in `~/.local/share/zaovra/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Zaovra insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Zaovra come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Zaovra al tuo account Sentry.
