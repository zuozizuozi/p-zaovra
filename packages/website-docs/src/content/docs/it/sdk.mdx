---
title: SDK
description: Client JS type-safe per il server zaovra.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`

L'SDK JS/TS di zaovra fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare zaovra in modo programmatico.
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `zaovra.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di zaovra in esecuzione, puoi creare un client per collegarti:
