---
title: Plugin
description: Scrivi plugin per estendere Zaovra.
---

I plugin ti permettono di estendere Zaovra agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Zaovra.
...
- `.zaovra/plugins/` - plugin a livello progetto
- `~/.config/zaovra/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/zaovra/node_modules/`.
...
1. Config globale (`~/.config/zaovra/zaovra.json`)
2. Config di progetto (`zaovra.json`)
3. Directory plugin globale (`~/.config/zaovra/plugins/`)
4. Directory plugin di progetto (`.zaovra/plugins/`)
...
Zaovra esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di zaovra per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere zaovra.
...
:::note
Se usi l'app desktop di Zaovra, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a zaovra di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a zaovra:
...
L'helper `tool` crea uno strumento personalizzato che zaovra può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in zaovra insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://zaovra.com/docs/sdk) per i dettagli.
