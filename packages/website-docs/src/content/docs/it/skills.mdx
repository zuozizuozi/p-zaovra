...
Le skill degli agenti permettono a Zaovra di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Zaovra cerca in queste posizioni:

- Config di progetto: `.zaovra/skills/<name>/SKILL.md`
- Config globale: `~/.config/zaovra/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Zaovra risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.zaovra/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/zaovra/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.zaovra/skills/git-release/SKILL.md` cosi':
...
Zaovra elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `zaovra.json`:
...
**Per agenti integrati** (in `zaovra.json`):
