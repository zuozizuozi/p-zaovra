...
With Zaovra you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Zaovra uses our own `zaovra` theme.
...
Zaovra comes with several built-in themes.
...
- Want Zaovra to match their terminal's appearance
...
Zaovra supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/zaovra/themes/*.json` or `$XDG_CONFIG_HOME/zaovra/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.zaovra/themes/*.json`
4. **Current working directory** - Defined in `./.zaovra/themes/*.json`
