...
Zaovra uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Zaovra with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
zaovra --auto
```

You can also use auto mode with [`zaovra run`](/docs/cli#run).

```bash
zaovra run --auto "Refactor this module"
```
...
```json title="zaovra.json"
{
  ...
  "permission": {
    ...
    "edit": {
      "*": "deny",
      "packages/website-docs/src/content/docs/*.mdx": "allow"
    }
  }
}
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Zaovra was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Zaovra permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Zaovra starts from permissive defaults:
...
When Zaovra prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Zaovra session)
