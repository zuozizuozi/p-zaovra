...
This is the living design and status document for `@zaovra-ai/codemode` and its existing V2 Zaovra adapter.
...
`packages/zaovra/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
