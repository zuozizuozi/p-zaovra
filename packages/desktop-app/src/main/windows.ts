...
import { resolveThemeVariant } from "@zaovra-ai/ui/theme/resolve"
import type { DesktopTheme } from "@zaovra-ai/ui/theme/types"
...
// Mirrors windowStorage() in packages/app-ui/src/utils/persist.ts, which names
// the per-window renderer store this window persists its tabs into.
function windowDataFile(id: string) {
  return `zaovra.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "Zaovra window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("Zaovra is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
