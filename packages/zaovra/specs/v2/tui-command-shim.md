...
Delete `packages/zaovra/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/zaovra/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/zaovra/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/zaovra`
- TUI plugin loader tests in `packages/zaovra` if runtime plugin API wiring changed
