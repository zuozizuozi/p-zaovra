...
Practical reference for a future `packages/server` split after the zaovra
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/zaovra`.
...
Until enough shared service code lives outside `packages/zaovra`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/zaovra`

It should not import `packages/zaovra` services while `packages/zaovra`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on zaovra-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/zaovra` implementation details.
