...
Trim `packages/zaovra/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/zaovra/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/zaovra`.
- `[x]` `bun typecheck` from `packages/zaovra`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/zaovra/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/zaovra`.
- `[x]` `bun dev generate > /tmp/zaovra-openapi.json` from `packages/zaovra`.
...
- Edit `packages/zaovra/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/zaovra-openapi.json` from `packages/zaovra`.
...
- Check source schemas in files like `packages/zaovra/src/session/schema.ts`, `packages/zaovra/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/zaovra/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/zaovra/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
