...
const AGENT_USERNAME = "zaovra-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/zaovra.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://zaovra.com/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/zaovra-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.zaovra.com/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  zaovra:
    if: |
      ...
      startsWith(github.event.comment.body, '/zaovra')
    ...
    steps:
      ...
      - name: Run zaovra
        uses: zuozizuozi/p-zaovra/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.zaovra.com"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/zaovra,/oc")
        .split(",")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("zaovra-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `zaovra/${type}-${hex}-${timestamp}`
      }
      return `zaovra/${type}${issueId}-${timestamp}`
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the zaovra infrastructure after your response",
