...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for zaovra serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ZaovraHttpApi)
...
export const PublicApi = ZaovraHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "zaovra",
    version: "1.0.0",
    description: "zaovra api",
