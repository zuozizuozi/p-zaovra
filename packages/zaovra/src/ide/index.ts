...
import { IdeEvent } from "@zaovra-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["ZAOVRA_CALLER"] === "vscode" || process.env["ZAOVRA_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.zaovra"], {
    nothrow: true,
  })
