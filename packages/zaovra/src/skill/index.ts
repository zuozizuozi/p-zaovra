...
import { Glob } from "@zaovra-ai/core/util/glob"
...
const ZAOVRA_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with zaovra. The model's intuition for what an
// zaovra.json should look like is often wrong, and zaovra hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch zaovra's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_ZAOVRA_SKILL_NAME = "customize-zaovra"
const CUSTOMIZE_ZAOVRA_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating zaovra's own configuration: zaovra.json, zaovra.jsonc, files under .zaovra/, or files under ~/.config/zaovra/. Also use when creating or fixing zaovra agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring zaovra itself."
const CUSTOMIZE_ZAOVRA_SKILL_BODY = SkillPlugin.CustomizeZaovraContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, ZAOVRA_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@zaovra/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_ZAOVRA_SKILL_NAME] = {
          name: CUSTOMIZE_ZAOVRA_SKILL_NAME,
          description: CUSTOMIZE_ZAOVRA_SKILL_DESCRIPTION,
          location: "<built-in>",
