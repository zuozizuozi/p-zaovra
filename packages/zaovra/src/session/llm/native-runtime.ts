...
import type { LLMClientShape } from "@zaovra-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("zaovra"))
    return { type: "unsupported", reason: "provider is not openai, zaovra, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @zaovra-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains zaovra-owned. The native runtime only adapts
      // the @zaovra-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
