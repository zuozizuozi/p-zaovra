...
export const DEPRECATED_PLUGIN_PACKAGES = ["zaovra-openai-codex-auth", "zaovra-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, zaovraVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(zaovraVersion) || semver.major(zaovraVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.zaovra
  if (typeof range !== "string") return
  if (!semver.satisfies(zaovraVersion, range)) {
    throw new Error(`Plugin requires zaovra ${range} but running ${zaovraVersion}`)
  }
}
