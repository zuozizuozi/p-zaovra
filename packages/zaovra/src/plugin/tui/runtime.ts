...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@zaovra-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@zaovra-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@zaovra-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@zaovra-ai/tui/context/theme"
import { Global } from "@zaovra-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@zaovra-ai/core/util/flock"
import { Flag } from "@zaovra-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@zaovra-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@zaovra-ai/core/v1/config/plugin"
import { createCommandShim } from "@zaovra-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@zaovra-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".zaovra"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".zaovra", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".zaovra", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ZAOVRA_PURE ? [] : pluginOrigins
    if (Flag.ZAOVRA_PURE && pluginOrigins.length) {
    }
