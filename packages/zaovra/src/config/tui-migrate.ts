...
const TUI_SCHEMA_URL = "https://zaovra.com/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from zaovra.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const zaovra = await zaovraFiles(input)
  for (const file of zaovra) {
...
async function zaovraFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "zaovra"),
    ...(await Filesystem.findUp(["zaovra.json", "zaovra.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zaovra"))
  }
  if (Flag.ZAOVRA_CONFIG) files.push(Flag.ZAOVRA_CONFIG)
