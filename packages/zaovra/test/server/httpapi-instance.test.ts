...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-zaovra-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.ZAOVRA_WORKSPACE_ID
      Flag.ZAOVRA_WORKSPACE_ID = WorkspaceV2.ID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.ZAOVRA_WORKSPACE_ID = originalWorkspaceID
        }),
      )
      ...
      const response = yield* Effect.promise(() =>
        HttpApiApp.webHandler().handler(
          new Request(`http://localhost/project/${projectID}`, {
            method: "PATCH",
            headers: { "x-zaovra-directory": dir, "content-type": "application/json" },
