      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-zaovra-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://zaovra.com/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-zaovra-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-zaovra-workspace")
