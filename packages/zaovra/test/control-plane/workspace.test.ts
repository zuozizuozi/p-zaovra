...
import { EventSequenceTable } from "@zaovra-ai/core/event/sql"
...
const originalEnv = {
  ZAOVRA_AUTH_CONTENT: process.env.ZAOVRA_AUTH_CONTENT,
  ZAOVRA_EXPERIMENTAL_WORKSPACES: process.env.ZAOVRA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.ZAOVRA_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@zaovra.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.ZAOVRA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=zaovra-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.ZAOVRA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.ZAOVRA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.ZAOVRA_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=zaovra-test")
