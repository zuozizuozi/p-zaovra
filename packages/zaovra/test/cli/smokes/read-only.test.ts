...
describe("zaovra read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // ZAOVRA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["mcp", "list"])
        zaovra.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["providers", "list"])
        zaovra.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["models"])
        zaovra.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["agent", "list"])
        zaovra.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh ZAOVRA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["session", "list"])
        zaovra.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["stats"])
        zaovra.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under ZAOVRA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ zaovra }) =>
      Effect.gen(function* () {
        const r = yield* zaovra.spawn(["db", "path"])
        zaovra.expectExit(r, 0, "db path")
