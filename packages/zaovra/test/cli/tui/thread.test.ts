...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ zaovra }) =>
    Effect.gen(function* () {
      const result = yield* zaovra.spawn(["--replay-limit", "10"])
  ...
  cliIt.live("routes attached sessions to mini mode", ({ zaovra }) =>
    Effect.gen(function* () {
      const result = yield* zaovra.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ zaovra }) =>
    Effect.gen(function* () {
      const result = yield* zaovra.spawn(["--mini", "--port", "4096"])
