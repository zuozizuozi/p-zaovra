// Subprocess test harness for the zaovra CLI. Spawns the real binary against
...
//
// Configuration flows through zaovra's built-in test affordances:
//   - ZAOVRA_CONFIG_CONTENT      : provider config inline, no files to find
//   - ZAOVRA_TEST_HOME           : pins os.homedir() → tmpdir
//   - ZAOVRA_DISABLE_PROJECT_CONFIG : skip walking up for zaovra.json
//   - ZAOVRA_PURE                : skip external plugin discovery + install
//   - ZAOVRA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `zaovra.run` is fully wired. The shape supports adding more
// builders (`zaovra.serve(opts)`, `zaovra.acp(opts)`, `zaovra.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ZaovraCli.
...
const zaovraRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(zaovraRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ZAOVRA_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    ZAOVRA_CONFIG_CONTENT: configJson,
    ZAOVRA_DISABLE_PROJECT_CONFIG: "1",
    ZAOVRA_PURE: "1",
    ZAOVRA_DISABLE_AUTOUPDATE: "1",
    ZAOVRA_DISABLE_AUTOCOMPACT: "1",
    ZAOVRA_DISABLE_MODELS_FETCH: "1",
    ZAOVRA_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `zaovra run`. New flags should
...
// `zaovra serve` is a long-lived process — it never exits on its own.
...
// `zaovra acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ZaovraCli = {
  ...
  // Spawn `zaovra serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `zaovra acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("zaovra.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ZAOVRA_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("zaovra.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("zaovra.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "zaovra server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `zaovra serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("zaovra.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const zaovra: ZaovraCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, zaovra })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `zaovra.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "zaovra") {
...
// every call site. Use this for any test that needs the zaovra CLI fixture.
//
...
// resources (e.g. `zaovra.serve`) without an extra `Effect.scoped` wrapper —
