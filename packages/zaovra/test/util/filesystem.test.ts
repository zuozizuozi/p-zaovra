...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "zaovra.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "zaovra.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zaovra.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zaovra.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["zaovra.json", "zaovra.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "zaovra.json"),
        path.join(project, "zaovra.jsonc"),
      ])
    })
  })
