...
import { ZaovraPlugin } from "@zaovra-ai/core/plugin/provider/zaovra"
...
const addPlugin = Effect.fn(function* () {
  ...
  yield* ZaovraPlugin.effect(host).pipe(
...
describe("ZaovraPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("zaovra")))?.methods).toEqual([
        {
  ...
  it.live("loads providers and models from the connected Zaovra server", () =>
      ...
      Effect.gen(function* () {
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("zaovra"),
            method: { type: "env", names: ["CUSTOM_ZAOVRA_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("ignores non-zaovra providers and models", () =>
    withEnv({ ZAOVRA_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the zaovra small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
