...
import { GoogleVertexPlugin } from "@zaovra-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.zaovra, (provider) => {
          provider.api = {
            ...
            url: "https://zaovra.com/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.zaovra))
