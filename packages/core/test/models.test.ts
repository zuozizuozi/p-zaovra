...
// test/preload.ts pins ZAOVRA_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.ZAOVRA_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.ZAOVRA_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.ZAOVRA_MODELS_PATH = undefined
  Flag.ZAOVRA_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.ZAOVRA_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.ZAOVRA_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
