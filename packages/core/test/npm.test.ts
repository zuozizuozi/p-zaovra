...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@zaovra/acme")).toBe("@zaovra/acme")
    expect(Npm.sanitize("@zaovra/acme@1.0.0")).toBe("@zaovra/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/zaovra/acme.git"
    const expected = win ? "acme@git+https_//github.com/zaovra/acme.git" : spec
