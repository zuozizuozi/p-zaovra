...
export async function create(input: {
  serverID: string
  server: { process: ChildProcessWithoutNullStreams; initialization?: Record<string, unknown> }
  root: string
  directory: string
  runtime: {
  ...
  function getFilePath(uri: string) {
    ...
    return runtime.normalizePath(fileURLToPath(uri))
  }
  ...
  input.server.process.once("exit", () => connection.dispose())
  input.server.process.once("close", () => connection.dispose())
  input.server.process.once("error", () => connection.dispose())
  ...
  // Deleted files can no longer be canonicalized through realpath on Windows.
  const deletionKey = (file: string) => (process.platform === "win32" ? file.toLowerCase() : file)
  ...
  const updatePullDiagnostics = (filePath: string, next: Diagnostic[]) => {
    if (deleted.has(deletionKey(filePath))) return
  ...
  connection.onNotification("textDocument/publishDiagnostics", (params) => {
    ...
    if (!filePath || deleted.has(deletionKey(filePath))) return
  ...
  const result = {
    ...
    notify: {
      async open(request: { path: string }) {
        request.path = runtime.normalizePath(
          path.isAbsolute(request.path) ? request.path : path.resolve(input.directory, request.path),
        )
        const text = await runtime.readText(request.path)
        deleted.delete(deletionKey(request.path))
      ...
      async remove(request: { path: string }) {
        const file = runtime.normalizePath(
          path.isAbsolute(request.path) ? request.path : path.resolve(input.directory, request.path),
        )
        const opened = files[file] !== undefined
        deleted.add(deletionKey(file))
        delete files[file]
        pushDiagnostics.delete(file)
        pullDiagnostics.delete(file)
        ...
        await connection.sendNotification("workspace/didChangeWatchedFiles", {
          changes: [{ uri: pathToFileURL(file).href, type: FILE_CHANGE_DELETED }],
        })
      },
    },
    get diagnostics() {
    ...
    async waitForDiagnostics(request: { path: string; version: number; mode?: "document" | "full"; after?: number }) {
      const normalizedPath = runtime.normalizePath(
        path.isAbsolute(request.path) ? request.path : path.resolve(input.directory, request.path),
      )
