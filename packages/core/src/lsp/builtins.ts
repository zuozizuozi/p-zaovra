export * as LSPBuiltins from "./builtins"
...
export interface Runtime {
  filesystem: {
    up(options: { targets: string[]; start: string; stop?: string }): AsyncGenerator<string, void, unknown>
    findUp(target: string, start: string, stop?: string): Promise<string[]>
    exists(file: string): Promise<boolean>
    readText(file: string): Promise<string>
    write(file: string, content: Buffer): Promise<void>
    writeStream(file: string, stream: ReadableStream<Uint8Array>): Promise<void>
  }
  process: {
    run(command: string[], options?: Options): Promise<ProcessResult>
    text(command: string[], options?: Options): Promise<ProcessResult & { text: string }>
    spawn(command: string[], options?: Options): ChildProcess & { exited: Promise<number> }
  }
  archive: { extractZip(file: string, destination: string): Promise<void> }
  spawn: {
    (command: string, args: string[], options?: Options): ChildProcessWithoutNullStreams & { exited: Promise<number> }
    (command: string, options?: Options): ChildProcessWithoutNullStreams & { exited: Promise<number> }
  }
}
...
type RootFunction = (file: string, ctx: Context) => Promise<string | undefined>
...
export interface Info {
  ...
  global?: boolean
  root: RootFunction
  spawn(root: string, ctx: Context, flags: Flags): Promise<Handle | undefined>
}
...
export function create(runtime: Runtime) {
  ...
  const pathExists = async (p: string) =>
    fs
      ...
      .then(() => true)
      .catch(() => false)
  const run = (cmd: string[], opts: Options = {}) => Process.run(cmd, { ...opts, nothrow: true })
  const output = (cmd: string[], opts: Options = {}) => Process.text(cmd, { ...opts, nothrow: true })
  ...
  const Clangd: Info = {
    ...
    async spawn(root, _ctx, flags) {
      ...
      const release = Schema.decodeUnknownSync(Release)(await releaseResponse.json())
  ...
  const JDTLS: Info = {
    ...
    async spawn(root, _ctx, flags) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaovra-jdtls-data"))
      return {
        process: spawn(
          java,
          [
