...
export class ConfigService extends Context.Service<ConfigService, Config>()("@zaovra/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ZAOVRA_WEBSEARCH_PROVIDER === "exa" || process.env.ZAOVRA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ZAOVRA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ZAOVRA_EXPERIMENTAL") || truthy("ZAOVRA_ENABLE_EXA") || truthy("ZAOVRA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ZAOVRA_ENABLE_PARALLEL") || truthy("ZAOVRA_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `zaovra/${InstallationVersion}`,
