...
import type { IntegrationOAuthMethodRegistration } from "@zaovra-ai/plugin/v2/effect/integration"
import { define } from "@zaovra-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.zaovra.com"
const clientID = "zaovra-cli"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("zaovra"),
    method: {
      id: methodID,
      type: "oauth",
      label: "Zaovra Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const ZaovraPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "zaovra",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ZaovraPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("zaovra")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Zaovra provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("zaovra", (integration) => {
        integration.name = "Zaovra"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "zaovra", method: { type: "key", label: "API key (service account)" } })
    })
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("zaovra")),
