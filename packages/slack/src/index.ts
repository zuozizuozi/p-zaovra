...
import { createZaovra, type ToolPart } from "@zaovra-ai/sdk"
...
console.log("🚀 Starting zaovra server...")
const zaovra = await createZaovra({
  port: 0,
})
console.log("✅ Zaovra server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await zaovra.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new zaovra session...")
    const { client, server } = zaovra
    ...
    console.log("✅ Created zaovra session:", createResult.data.id)
  ...
  console.log("📝 Sending to zaovra:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Zaovra response:", JSON.stringify(result, null, 2))
