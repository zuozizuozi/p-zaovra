# Zaovra V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@zaovra-ai/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an Zaovra extension point.
...
```ts
import { define } from "@zaovra-ai/plugin/v2/promise"
