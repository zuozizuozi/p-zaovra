...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.zaovra.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `zaovra.settings.dat` is special: it maps to the `zaovra.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"zaovra.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // zaovra.settings.dat → the electron settings store ("zaovra.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "zaovra.global.dat").
  const storeName = filename === "zaovra.settings.dat" ? "zaovra.settings" : filename
