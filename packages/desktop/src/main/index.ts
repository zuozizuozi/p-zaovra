...
const TEST_ONBOARDING = process.env.ZAOVRA_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.ZAOVRA_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.zaovra.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `zaovra-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Zaovra Dev")
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.ZAOVRA_PORT
    if (fromEnv) {
