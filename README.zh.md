> 本地开发请先看 [目录说明](./目录说明.md)：官网在 `packages/website`，文档站在 `packages/website-docs`，桌面端在 `packages/desktop-app`，工作台界面在 `packages/app-ui`。
...
brew install zuozizuozi/tap/zaovra # macOS 和 Linux（推荐，始终保持最新）
brew install zaovra              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S zaovra            # Arch Linux (Stable)
paru -S zaovra-bin               # Arch Linux (Latest from AUR)
mise use -g zaovra               # 任意系统
nix run nixpkgs#zaovra           # 或用 github:zuozizuozi/p-zaovra 获取最新 dev 分支
```
...
Zaovra 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/zuozizuozi/p-zaovra/releases) 或 [zaovra.com/download](https://zaovra.com/download) 下载。
...
1. `$ZAOVRA_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.zaovra/bin` - 默认备用路径
...
Zaovra 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://zaovra.com/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://zaovra.com/docs)。
...
如果你在项目名中使用了 “zaovra”（如 “zaovra-dashboard” 或 “zaovra-mobile”），请在 README 里注明该项目不是 Zaovra 团队官方开发，且不存在隶属关系。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/zaovra)
